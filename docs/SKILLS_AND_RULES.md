...
在 Codex 环境中执行 `git push` 时，审批服务可能在 Git 命令真正执行前因连接中断而拒绝操作。遇到此类错误时：

1. 不把审批服务错误误判为 Git 或 GitHub 错误，也不修改提交历史、远端地址或认证配置。
2. 先执行 `git status --short --branch`，确认本地分支仍领先远端；必要时用 `git log -3 --oneline` 核对待推送提交。
3. 向用户说明失败发生在审批阶段，并在用户再次明确授权后原样重试 `git push origin <branch>`。
4. 推送成功后再次检查分支状态；只有本地不再领先远端时，才报告推送完成。
5. 禁止通过其他 shell、脚本、GitHub API 或替代命令绕过被拒绝的推送审批。
