...
2026-08-07 继续优化 Simple routing 与共享空间索引：

- Simple router 对每个候选路径只计算一次冲突、折点和长度评分，避免筛选与排序重复查询空间索引。
- 候选选择改为基于缓存评分的稳定线性扫描；评分相同时仍保留原候选顺序。
- `SpatialHashIndex` 使用数值 column/row bucket，避免为每次插入和查询分配字符串 cell key。
- 单 bucket 查询增加快速路径，多 bucket 查询继续使用 `Set` 保持去重语义。

同一环境下，对 8K 深链运行十次并取中位数：

| 指标 | 第二批前 | 第二批后 | 变化 |
| --- | ---: | ---: | ---: |
| 完整布局 | 212.3 ms | 160.5 ms | 降低约 24% |
