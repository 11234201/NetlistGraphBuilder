...
平移距离按 SVG 实际的等比例缩放换算；即使超宽或超高图已经放到最大，鼠标拖动多少，画面也会按相同的屏幕距离移动。

超宽或超高图会按实际画布宽高自动提高放大倍率上限；上限对应典型 220px 宽 cell 在屏幕中约 440px，约为布局尺寸的两倍。低缩放时，为保持性能和轮廓可读性，pin、net label、cell metadata 和 Timing 文本可能暂时隐藏；放大后会恢复。
